...
all_data_diagnosis = {
    'diagnosis_ori': ('ринит', "синусит", "отит", "вирусная экзантема",
                      ...
                      "улучшение", 'реконвалесцент'),
    'diagnosis_ФРК': ("улучшение", 'реконвалесцент'),
    'diagnosis_Ветрянка': ("улучшение", 'реконвалесцент'),
...
class ExaminationRoot(tk.Toplevel):
    def __init__(self):
        ...
        self.canvas = tk.Canvas(self, height=self.winfo_screenheight(),
                ...
                def select_type_ln():
                    ...
                    if type_ln in ("Справка ВН", "Лист ВН"):
                        ...
                        if found_info:
                            ...
                            ln_num = f"{last_ln_num.replace(ln_num_digit, '')}" \
    ...
    def resize(self, event):
        ...
        self.minsize(int(self.examination_root.winfo_width()), self.winfo_height())
...
def examination_cmd_main():
    ...
    def create_examination_doc():
        type_ln = selected_type_ln.get().replace(' ', '')
        if type_ln in ('Лист ВН', 'Справка ВН') and not txt_ln_num.get():
            messagebox.showerror('Ошибка!', 'Не указан номер документа ВН!')
            txt_ln_num.focus()
        data_base_info = list()
        ...
        if type_ln == 'Уход обеспечен':
            render_data['prescription'] = f"{render_data.get('prescription')}\nУход обеспечен"

        elif type_ln in ('Лист ВН', 'Справка ВН'):
            text = f"{type_ln} №{txt_ln_num.get()} c {txt_ln_from} по {txt_ln_until}"
            render_data['prescription'] = f"{render_data.get('prescription')}\n{text}"

            # with sq.connect('data_base.db') as conn:
            #     cur = conn.cursor()
            #     cur.execute(f"SELECT ln_num FROM врачи WHERE doctor_name LIKE '{user.get('doctor_name')}'")
            #     last_ln_num = cur.fetchone()[0]
            # print(last_ln_num)
            # if not last_ln_num:
            #     last_ln_num = '0__0'
            #
            # if type_ln == 'Лист ВН':
            #     new_ln_num = f"{txt_ln_num.get()}__{last_ln_num.split('__')[1]}"
            # else:
            #     new_ln_num = f"{last_ln_num.split('__')[0]}__{txt_ln_num.get()}"
            #
            # cur.execute(f"SELECT doctor_name, district, ped_div, manager, text_size, ln_num FROM врачи "
            #             f"WHERE doctor_name LIKE '{user.get('doctor_name')}'")
            # last_ln_num = cur.fetchone()[0]
        ...
        #     data['examination']['last_ln_num'] = last_ln_num
        #     if type_ln == 'Лист ВН':
        #         ln_num = last_ln_num.split('__')[0]
        #         ln_num_digit = ''
        #         for word in ln_num:
        #             if word.isdigit():
        #                 ln_num_digit += word
        #         ln_num = f"{ln_num.replace(ln_num_digit, '')}{int(ln_num_digit) + 1}"
        #
        #     elif type_ln == 'Справка ВН':
        #
        #         ln_num = last_ln_num.split('__')[1]
        #         ln_num_digit = ''
        #         for word in ln_num:
        #             if word.isdigit():
        #                 ln_num_digit += word
    ...
    def paste_frame_ln():
        ...
        def select_type_ln():
            ...
            if type_ln in ("Справка ВН", "Лист ВН"):
                lbl_type_ln['text'] = f"{type_ln} с"
                frame_ln_add.grid(row=0, column=3, sticky='ew')
            else:
                frame_ln_add.grid_remove()
                # Label(master=frame_ln, text=f"{ln_num}",
                #       font=('Comic Sans MS', user.get('text_size')), bg='white').grid(row=0, column=3, sticky='ew',
                #                                                                       columnspan=2)
            frame_type_ln.columnconfigure(index='all', minsize=40, weight=1)
            frame_type_ln.rowconfigure(index='all', minsize=20)
            frame_type_ln.grid(row=0, column=4, sticky='ew')
        ...
        Label(master=frame_ln_add, text="номер:",
              font=('Comic Sans MS', user.get('text_size')), bg='white').grid(row=0, column=0, sticky='ew')
        txt_ln_num.grid(row=0, column=1, sticky='ew')

        lbl_type_ln.grid(row=0, column=2, sticky='ew')
    ...
    frame_ln_main = Frame(examination_root, relief="solid", padx=1, pady=1)
    frame_ln = Frame(frame_ln_main, relief="solid", padx=1, pady=1)
    frame_ln_add = Frame(frame_ln, relief="solid", padx=1, pady=1)
    frame_second_examination = Frame(frame_ln_main, relief="solid", padx=1, pady=1)
    ...
    txt_ln_from = Entry(frame_ln_add, width=15, font=('Comic Sans MS', user.get('text_size')))
    txt_ln_until = Entry(frame_ln_add, width=15, font=('Comic Sans MS', user.get('text_size')))
    txt_second_examination = Entry(frame_second_examination, width=15, font=('Comic Sans MS', user.get('text_size')))
    lbl_type_ln = Label(master=frame_ln_add, text=f"",
          font=('Comic Sans MS', user.get('text_size')), bg='white')
    ...
    def paste_calendar(text_field):
        ...
        lbl_month_year = Label(frame_month_year, text="", font=('Comic Sans MS', user.get('text_size')), bg='white')
        lbl_month_year.grid(column=1, row=0, sticky='ew')
