...
class TtkCalendar(ttk.Frame):
    ...
    def create_header(self):
        ...
        btn_right = ttk.Button(hframe, style='R.TButton',
                               command=lambda: self.move_month(1))
        label = ttk.Label(hframe, width=15, anchor='center', font=('Comic Sans MS', 20))
    ...
    def create_table(self):
        ...
        width = max(map(self.font.measure, cols)) + 20
        for col in cols:
    ...
    def build_calendar(self):
        ...
        for name in month_names:
            if name in month_name:
                month_name = month_name.replace(name, month_names.get(name))
        ...
        for week, item in zip_longest(month_weeks, items):
            ...
            if year == datetime.datetime.now().year and month == datetime.datetime.now().month:
                now_date = '%02d' % datetime.datetime.now().day
                if now_date in fmt_week:
                    fmt_week[fmt_week.index(now_date)] = f"<{now_date}>"


            print('fmt_week', fmt_week)
    ...
    def pressed(self, event):
        ...
        if not column or not item in items:
            print("клик на заголовок или за пределами столбцов")
...
def main():
    ...
    ttkcal = TtkCalendar(firstweekday=calendar.MONDAY)
